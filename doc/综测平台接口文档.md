## 1. 当前实现状态说明（2026-02-26）

以下接口已在当前后端代码中实现：

- 认证：`/auth/register`、`/auth/login`、`/auth/refresh`、`/auth/logout`
- 令牌：`/tokens/reviewer`、`/tokens/reviewer/activate`、`/tokens`、`/tokens/{token_id}/revoke`
- 申报：`/applications`、`/applications/my/category-summary`、`/applications/my/by-category`、`/applications/{application_id}`、`/applications/{application_id}/withdraw`、`DELETE /applications/{application_id}`
- 人工审核（审核员/教师）：`/reviews/pending`、`/reviews/pending/category-summary`、`/reviews/pending/by-category`、`/reviews/{application_id}`、`/reviews/{application_id}/decision`、`/reviews/batch-decision`、`/reviews/history`、`/reviews/pending-count`
- 文件：`/files/upload`、`/files/{file_id}`
- 导出（教师）：`POST /teacher/exports`、`GET /teacher/exports/{task_id}`、`GET /teacher/exports/{task_id}/download`
- 导出归档：`POST /archives/exports`、`GET /archives/exports`、`GET /archives/exports/{archive_id}`、`GET /archives/exports/{archive_id}/download`
- 公示：`POST /announcements`、`GET /announcements`

以下接口仍属于规划项（文档保留，但当前未实现）：AI 审核、教师管理（除导出）、邮件通知、系统基础管理、申诉处理等模块。
...
- **Method**: `GET`
- **权限**: `teacher/admin`
- **Query**: `type=reviewer&page=1&size=10&status=active|expired|revoked`
    ...
    "list": [
      {
        ...
        "score": 3.0,
        "created_at": "2026-02-13T10:03:30Z"
      }
    ]
  }
}
...
> 当前实现说明：`/reviews/*` 为统一人工审核接口。
>
> - 审核员：账号角色仍为 `student`，通过激活审核员令牌后 `is_reviewer=true` 获得审核权限；数据范围受令牌 `class_ids` 限制；可审核状态为 `pending_review` / `ai_abnormal`。
> - 教师（含管理员）：可访问全局数据（支持 `class_id` 过滤）；可审核状态为 `pending_teacher`。

### 1. 获取待审核列表（按管辖班级）

- **URL**: `/reviews/pending`
- **Method**: `GET`
- **权限**: `reviewer/teacher/admin`
...
```json
{
  ...
  "data": {
    ...
    "status": "pending_teacher",
    "review_id": 19,
    "reviewed_at": "2026-02-13T11:03:30Z"
  }
}
```
...
```json
{
  ...
  "data": {
    ...
    "status": "approved",
    "review_id": 20,
    "reviewed_at": "2026-02-13T12:03:30Z"
  }
}
```
...
```json
{
  "application_ids": [9012, 9013, 9014],
  "decision": "approved",
  "comment": "材料齐全，符合条件",
  "reason_code": null,
  "reason_text": null
...
- **Method**: `GET`
- **权限**: `reviewer/teacher/admin`
- **Query**: `class_id=&result=approved|rejected&from=&to=&page=&size=`
...
```json
{
  "export_task_id": "exp_10001", // 直接绑定到某次“导出”的export_task_id上，以下的信息就不传了
...
```json
{
  ...
  "message": "上传成功",
  "data": {
    "file_id": "7ac66c0f148de9519b8bd264312c4d64.pdf",
    "filename": "证书.pdf",
    "content_type": "application/pdf",
    "size": 213456,
    "url": "http://127.0.0.1:8000/api/v1/files/7ac66c0f148de9519b8bd264312c4d64.pdf"
  }
}
```
...
**推荐状态流转**：
`pending_ai -> (pending_review | ai_abnormal) -> (pending_teacher | rejected) -> (approved | rejected) -> archived`
...
### 申报
- `POST /applications`
- `GET /applications/my`（规划）
- `GET /applications/categories`（规划）
