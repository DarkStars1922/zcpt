...
> 说明：
> - 系统无单独 `reviewer` 账号角色。
> - 学生激活成功后，将把该学生 `is_reviewer` 设为 `true`，并绑定 `reviewer_token_id`。
> - 后续审核数据权限范围基于该绑定令牌对应的 `class_ids` 进行判定。
...
> 失活联动规则：
> - 当令牌被撤销（revoke）或过期（expired）时，会自动将对应学生 `is_reviewer` 置为 `false`，并清空 `reviewer_token_id`。
> - 被失活令牌绑定的学生将不再拥有审核员数据权限。
...
- **Method**: `GET`
- **权限**: `student/reviewer/teacher/admin`（按数据权限控制）
...
- **URL**: `/reviews/class-unsubmitted`
- **Method**: `GET`
- **权限**: `reviewer`
- **Query**: `class_id=&term=&page=&size=`
...
- **URL**: `/notifications/email-logs/{log_id}/resend`
- **Method**: `POST`
- **权限**: `teacher/admin`
...
- 学生：仅访问本人数据（申报、文件、申诉）
- 审核员能力：由学生激活审核员令牌后获得（`is_reviewer=true`），仅访问绑定令牌对应班级数据
- 教师：访问全局数据（可二次复核、统计、导出）
- 管理员：系统级配置与日志权限
...
- `POST /notifications/email-logs/{log_id}/resend`
...
- 增补 OpenAPI 3.0（Swagger）文档导出
- 增补各字段详细枚举字典（award_type、reason_code 等）
- 增补 webhook/消息队列异步回调协议
- 增补错误码国际化（中文/英文）
