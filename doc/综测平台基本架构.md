...
当前后端运行时不读取 `frontend` 内任何文件。

以下数据已下沉到后端自有目录：

- `app/data/award_score_map.json`
- `app/data/award_tree.json`

因此后续即使前端目录迁移或删除，后端仍可独立运行、迁移、测试和部署。

## 10. 已完成验证

截至 2026-04-02，已完成以下真实验证：

- SQLite 全链路自动化测试通过
- Redis 真实集成测试通过
- MySQL 真实迁移与 API smoke 测试通过
