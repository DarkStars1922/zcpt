...
- Celery：文件 OCR 分析、AI 审核、邮件、导出任务
...
- 上传成功后立即写入 `queued` 文件分析记录，并通过 Celery 触发 OCR 分析任务
...
- 学生端公示下载不暴露原始归档全量文件，而是实时生成只含官方总分的公示专用表；公示范围内已通过/已归档申报通过学生端“公示申报”入口单独查看
...
- 教师或管理员可按年级生成当前秋季填报学期的综测画像；年级必选，班级支持多选，不选班级表示全年级非毕业班
- 后端聚合学生四大类八小类分数、原始申报数、有效申报数、微分条目数、待处理数量、驳回数量、成果/突破覆盖情况
- 每条申报按 `award_uid -> rule_path/id_path` 解析层级，提取大类、小类、关键类别、获奖层级和成员角色
- `score <= 0.35`、参与未获奖、其他成员/其他作者等归为微分/参与痕迹，单独统计，不与成果类高价值申报混算
- 发送给大模型的数据只包含 `S0001` 这类匿名编号、班级编号以及清洗后的分数/申报统计，不包含学生姓名、学号、手机号、邮箱等身份信息
- 大模型返回匿名编号后，后端依据本地映射回填真实姓名、学号和班级给教师端展示，老师无需手动查表
- 分析结果包含整体画像、各班级分项画像、四个素养模块的积极性判断、需关注学生、证据、沟通建议、数据质量提醒和班级行动计划
- 若大模型未配置或超时失败，后端使用本地规则生成兜底分析，保证页面可用
...
POST /files/upload
  -> 保存文件到 uploads/
  -> 写入 file_analysis_result(status=queued)
  -> 入队 PaddleOCR 文件分析任务
...
POST /ai-audits/image-authenticity
  -> 校验当前用户可访问该证明文件
  -> 执行文件格式 / 元数据一致性检查
  -> 可选调用 c2patool 校验 C2PA / Content Credentials
  -> 可选调用外部高精度鉴别服务
  -> 返回独立风险结论，不写回申报业务状态
```
...
- TruFor：作为 P 图/篡改定位主模型，输出整图完整性分、局部热力图和可疑区域。
- UniversalFakeDetect + AI-GenBench 校准：作为通用 AI 生成图检测主线，结合校内真实奖状、截图、PDF 拼接、二次压缩样本重新校准阈值。
- 接口采用证据融合口径：C2PA/水印属于强证据，篡改定位和 AI 生成检测属于概率证据，最终只输出风险提示，不自动驳回申报。
- 具体 GPU 混合部署、`.env` 配置、外部服务响应字段和联调命令见 `docs/图片真实性检测配置手册.md`。
